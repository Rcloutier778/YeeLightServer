        ...
        while True:
            try:
                ...
                if self.ping_res is not None:
                    ...
                    writeManualOverride(self.switch_room, datetime.timedelta(hours=4))
                    ...
                    if not ( phoneStatus and pcStatus ):
                        logger.info("Phone and/or pc is offline, keeping lights off.")
                        global_action('off', auto=True, autoset_auto_var=False)#, autoset_auto_var = not self.timer_wake)
                    else:
